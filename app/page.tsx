...
// --- HỆ THỐNG ICON SUPREME (MẢNH - VIỀN ĐEN MỜ) ---
const SupremeIcon = ({ name, size = 26, color = "#FFFFFF" }: { name: string, size?: number, color?: string }) => {
  const filterStyle = { filter: 'drop-shadow(0px 0px 1.5px rgba(0,0,0,1))' };
  const icons: any = {
    ...
    volume: <path d="M11 5L6 9H2v6h4l5 4V5zM15.54 8.46a5 5 0 0 1 0 7.07" />, // Đã thu nhỏ
...
export default function SupremeFinal() {
  ...
  return (
    <div style={{ position: 'relative', width: '100vw', height: '100vh', backgroundColor: '#000', overflow: 'hidden' }}>
      ...
      {/* CỘT PHẢI - CĂN CHỈNH GIÃN ĐỀU */}
      <div style={{ 
        position: 'absolute', right: '12px', bottom: '90px', 
        display: 'flex', flexDirection: 'column', alignItems: 'center', 
        justifyContent: 'space-between', height: '340px', zIndex: 100 
      }}>
        ...
        {/* MỤC 3: NÚT LOA THU NHỎ & MENU SỔ (3 NGĂN) */}
        <div style={{ position: 'relative' }}>
          <div onClick={() => setShowVolumeMenu(!showVolumeMenu)} style={{ cursor: 'pointer' }}>
            <SupremeIcon name="volume" size={24} /> {/* Kích thước 3/4 (~24px) */}
          </div>
          {showVolumeMenu && (
            <div style={{ 
              position: 'absolute', right: '45px', bottom: '0', backgroundColor: 'rgba(0,0,0,0.8)', 
              borderRadius: '8px', padding: '10px', border: '1px solid #555', display: 'flex', gap: '15px' 
            }}>
              <span style={{ fontSize: '12px' }}>🔊 High</span>
              <span style={{ fontSize: '12px' }}>🔉 Mid</span>
              <span style={{ fontSize: '12px' }}>🔇 Mute</span>
            </div>
          )}
        </div>
      ...
      {/* MỤC 1 & 2: CỤM TRÁI - SHOP TRÊN AVATAR, KÉO SÁT ĐÁY */}
      <div style={{ 
        position: 'absolute', bottom: '75px', left: '15px', zIndex: 100, 
        display: 'flex', flexDirection: 'column', gap: '8px' 
      }}>
        {/* Nút Shop #14 đặt TRÊN Avatar #13 */}
        <div style={{ 
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            ...
            <span style={{ fontSize: '9px', color: '#ff4444', border: '1px solid #ff4444', padding: '0px 5px', borderRadius: '3px', width: 'fit-content', marginTop: '2px' }}>+ follow</span>
          </div>
        </div>
      ...
      {/* MỤC 4: THANH ĐIỀU HƯỚNG ĐÁY - THU HẸP KHOẢNG CÁCH 2/3 */}
      <div style={{ 
        position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)',
        width: '85%', height: '70px', // Thu hẹp độ rộng toàn dải để icon gần nhau hơn
        ...
        <SupremeIcon name="cart" size={26} /> {/* #10 */}
        <SupremeIcon name="global" size={28} /> {/* #9 */}
        
        {/* Nút + ĐĂNG VIDEO (#8) Trung tâm */}
        <div style={{ 
        ...
        <SupremeIcon name="home" size={26} /> {/* #7 */}
        <SupremeIcon name="mail" size={26} /> {/* #6 */}
      </div>
